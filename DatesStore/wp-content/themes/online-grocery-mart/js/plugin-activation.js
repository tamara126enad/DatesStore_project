jQuery(document).ready(function($) {
    'use strict';
    var this_obj = online_grocery_mart_plugin_activate_plugin;

    $('#gutenberg_editor .plugin-activation-redirect a').addClass('ive-redirect-to-dashboard');

    $(document).on('click', '.online-grocery-mart-plugin-install', function(event) {
        ...
        wp.updates.installPlugin({
            ...
            success: function(data) {
                ...
                button.removeClass('button-secondary updating-message online-grocery-mart-plugin-install');
                button.addClass('button-primary online-grocery-mart-plugin-activate');
                button.trigger('click');
            },
            error: function(data) {
                jQuery('.online-grocery-mart-recommended-plugins .online-grocery-mart-activation-note').css('display','block');
    ...
    $(document).on('click', '.online-grocery-mart-plugin-activate', function(event) {
    ...
    jQuery('#lite_theme .ibtana-skip-btn').click(function(){
        ...
        jQuery('#lite_theme .online-grocery-mart-recommended-plugins').css('display','none');
    });

    jQuery('#block_pattern .ibtana-skip-btn').click(function(){
        var light_theme = jQuery(this).attr('get-start-tab-id');
        jQuery('.' + light_theme).css('display','block');
        jQuery('#block_pattern .online-grocery-mart-recommended-plugins').css('display','none');
    });
